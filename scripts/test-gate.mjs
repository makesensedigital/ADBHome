...
const makeSite = async () => {
  ...
  // LOCAL DIVERGENCE FROM THE TEMPLATE - ADB, 2026-08-13. A defect in v4.0.0, reported upstream.
  //
  // The filter exists to stop a repository-root copy from recursing into its own fixture
  // directory, and it is applied to the SOURCE path. When the source IS the vendored fixture -
  // the case v4.0.0 added this resolution for - every path under it contains `scripts/fixtures`,
  // starting with the root itself. `cp` therefore copies NOTHING, and the run dies on the first
  // `edit()` with an ENOENT on config.js.
  //
  // So the adopted-repository path the release introduced does not work as shipped: the vendored
  // fixture is resolved and then immediately excluded. Measured rather than reasoned about - the
  // copy produced zero entries.
  //
  // The fix keeps upstream behaviour byte-identical in the fallback case, and only stops the
  // filter from excluding the fixture from itself.
  //
  // THE TRIGGER TO DROP THIS: an upstream release whose fixture copy works when TEMPLATE is the
  // vendored directory. Then delete this block and take the template's file byte-for-byte.
  const fixtureIsSource = TEMPLATE === VENDORED;
  await cp(TEMPLATE, dir, {
    recursive: true,
    filter: (src) => fixtureIsSource || !/[\\/]scripts[\\/]fixtures/.test(src),
  });
