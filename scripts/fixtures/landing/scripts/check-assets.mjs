#!/usr/bin/env node
// Byte budgets, formats, orphans and third-party origins — Handbook §26 (Static Conversion Sites). Node stdlib only.
//
...
import { relative, join, dirname, basename, posix } from "node:path";
...
// A reference written as a quoted string — the ordinary shape in markup, JS and JSON.
const QUOTED = /["']([^"']*\.(?:css|js|png|jpe?g|webp|avif|gif|svg|woff2?))(?:[?#][^"']*)?["']/gi;

// A reference written as CSS `url(...)`, where the quotes are OPTIONAL and most commonly absent.
// A stylesheet is the single most likely place a typeface or a background image is referenced from,
// and §26 (Static Conversion Sites) actively recommends self-hosting typefaces — so a scan that cannot read this penalises the
// remedy the section advises. The extension is required INSIDE the pattern rather than checked
// afterwards, so `url(var(--x))` and `url(data:…)` produce nothing rather than a phantom path.
const CSS_URL =
  /\burl\(\s*['"]?([^'")\s]*\.(?:css|js|png|jpe?g|webp|avif|gif|svg|woff2?))(?:[?#][^'")\s]*)?['"]?\s*\)/gi;

/**
 * A raw reference as it appears in a file, resolved to a repository-relative path — or null when it
 * is not a local file at all.
 *
 * `fromDir` is the directory of the file the reference was written in, and it is load-bearing. A
 * relative `url(fonts/x.woff2)` inside `assets/fonts.css` means `assets/fonts/x.woff2`; reading it
 * as a root path produces TWO findings for one correct line — a file missing at the root, and the
 * real file looking like an orphan.
 */
const localise = (raw, fromDir, requireSeparator) => {
  raw = String(raw ?? "").trim();
  ...
  // A quoted string ending in a known extension is not necessarily a path. The measurement script
  // pushes an event literally named `"gtm.js"`, and reading that as a missing file is the check
  // firing on correct input. Requiring a separator is the cheap, exact distinction — but only for
  // quoted strings: a bare filename inside `url(...)` IS a reference, always.
  if (requireSeparator && !raw.includes("/")) return null;
  const resolved = posix.normalize(posix.join(fromDir || ".", raw));
  // Above the repository root is not something this architecture can serve.
...
for (const file of files.filter((f) => /\.(css|js|json|webmanifest|xml|txt|html)$/.test(f))) {
  const text = await read(file);
  const fromDir = relative(ROOT, dirname(file)).replace(/\\/g, "/");
  for (const m of text.matchAll(QUOTED)) {
    const target = localise(m[1], fromDir, true);
    if (target) referenced.add(target);
  }
  for (const m of text.matchAll(CSS_URL)) {
    const target = localise(m[1], fromDir, false);
    if (target) referenced.add(target);
  }
}
...
// firing on correct input, which teaches people to route around it (§20 — Toolchain, Code Quality and Dependency Management).
const FETCHING = [
