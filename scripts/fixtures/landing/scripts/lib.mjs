// Shared helpers for the delivery gate — Handbook §26 (Static Conversion Sites). Node stdlib only.
//
 ...
 *
 * `.gate-baseline.json` is in the skip list for a reason that is not tidiness. Its keys embed the
 * paths of the findings it records — `"check-assets|trustlogo/javascript/trustlogo.js": 1` — and
 * every one of them reads as a quoted string ending in a scanned extension. Without this entry
 * `check-assets` reports the baseline's own keys as files referenced but not committed, and
 * recording those new findings writes longer paths into the same file, which produces more of them
 * on the next run. The ratchet's first act would otherwise guarantee that it never converges.
 *
 * It belongs here rather than in `check-assets` so `check-config`'s placeholder scan is covered by
 * the same line: it reads every file, and a baseline key can contain any sentinel string.
 */
export const walk = async (root, skip = [".git", "node_modules", "scripts", ".github", ".gate-baseline.json"]) => {
  const out = [];
 ...
 *
 * §20 (Toolchain, Code Quality and Dependency Management): a lint's message is INPUT TO THE NEXT TURN, not a report. Each failure therefore states the
...
export const reporter = (checkName) => {
  ...
  return {
    fail: (what, fix, section = "§26 (Static Conversion Sites)") => failures.push({ what, fix, section }),
