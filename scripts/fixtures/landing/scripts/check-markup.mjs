#!/usr/bin/env node
// Indexability and the mobile render rules — Handbook §26 (Static Conversion Sites). Node stdlib only.
//
...
for (const file of html) {
  ...
  if (indexable) {
    ...
    // correct input, which teaches people to route around it (§20 — Toolchain, Code Quality and Dependency Management).
  ...
  // and because the message can name the section (§6c — Visual Consistency Rules).
  ...
  if (!/<a\b[^>]*class="[^"]*\bskip-link\b[^"]*"[^>]*href="#/.test(text)) {
    r.fail(
      ...
      "§6c (Visual Consistency Rules)",
    );
  }
  for (const m of text.matchAll(/<img\b(?![^>]*\balt=)[^>]*>/gi)) {
    r.fail(`${rel} — an <img> has no alt attribute`, "add alt text, or alt=\"\" if the image is decorative", "§6c (Visual Consistency Rules)");
    break;
  }
  ...
  for (const m of text.matchAll(/<button\b[^>]*aria-controls=[^>]*>/gi)) {
    if (!/aria-expanded=/.test(m[0])) {
      r.fail(`${rel} — a disclosure button has aria-controls but no aria-expanded`, "add aria-expanded and keep it in sync", "§6c (Visual Consistency Rules)");
    }
  }
}

// ---------------------------------------------------------------- the four render rules
// The four rules below are string matches over CSS, and they do not care where the CSS came from —
// so they have to read INLINE styles as well as stylesheets. A site whose CSS lives entirely in a
// <style> block is not an exotic case: it is close to the DEFAULT shape of the thing §26 (Static Conversion Sites) governs,
// because it is what one person building one page quickly produces. Reading only `.css` files there
// means three of the four rules run against an empty string and report nothing, and a check that
// silently covers nothing reads exactly like a check that passed — the §20 (Toolchain, Code Quality and Dependency Management) failure mode arriving
// from the quiet side. Found on the first real adoption: 660 lines of inline CSS, a
// `min-height: 100vh` on `body` that rule 1 exists to catch, and a green result.
const sources = [];
for (const file of css) {
  sources.push({ name: relative(ROOT, file).replace(/\\/g, "/"), css: await read(file) });
}
for (const file of html) {
  const rel = relative(ROOT, file).replace(/\\/g, "/");
  for (const m of (await read(file)).matchAll(/<style\b[^>]*>([\s\S]*?)<\/style>/gi)) {
    // Named as inline, so a finding points at the file somebody has to open rather than at a
    // stylesheet that does not exist.
    sources.push({ name: `${rel} (inline <style>)`, css: m[1] });
  }
}

const styles = sources.map((s) => s.css).join("\n");
const styleNames = sources.map((s) => s.name).join(", ") || "(no stylesheet)";
