...
## Adopting this on a site that already exists

The commands above assume a new site, which starts compliant. A site that predates the standard
does not, and failing its gate on everything is how a gate gets switched off in a week.

```bash
node scripts/ratchet.mjs --init     # record today's findings as the baseline, and commit it
```

From then on the gate carries what existed and **fails only on what is new**, so the site stops
getting worse immediately without the gate reporting a compliance it does not have. Fix something
and `--update` lowers the baseline — in the same commit as the fix, because a baseline left high
keeps headroom for the violation to come back.

The full procedure, including how to triage what the first run finds, is the
...
| `config.js` | **Every external identifier**, and nothing else may hold one. Also the consent decision, the conversion receiver, the measurement destination and the discoverability decision. |
...
a different host — not a reason to proceed quietly.

**Where that gets recorded is `config.controls`, not prose.** One key per row of §26's
control-placement table, each saying where the control lives — `edge`, `gate`, `provider`,
`document` or `absent` — with a reason required whenever it is `absent`, stated as a capability
rather than an intention. The gate fails on a row nobody answered, and it fails on `_headers` or
`_redirects` being committed while the rows say the host cannot read them: **a file that does
nothing reads as a control.** So on Pages you either delete those two files and record why, or you
move to a host that reads them. Six of the ten rows ship answered; the four left open are the ones
this decision actually settles.
