...
| Primary conversion | | | |
| Discoverability — found in search, or deliberately not | | | |
...
|---|---|---|---|
| 1 | Measurement contract, container and the destination that receives it | | |
...
- [ ] Events seen arriving **in the destination**, with intent and outcome distinct — a configured
      container is not a measurement
- [ ] Search property verified and the sitemap submitted **in it** — or the decision not to be found
      recorded with its owner, and `robots.txt` regenerated so the served file says so
