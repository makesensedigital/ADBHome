...
source_hash:
  ...
  22: 044d89d4f03ee9cf
-->
<!-- Generated from the Platform Engineering Handbook, Section 26 (Static Conversion Sites). Do not edit by hand. -->
...
This is the condensed form of **Handbook §26 (Static Conversion Sites)**, the only section that governs this class of
...
logging, the five build-and-test stages of the quality gate. Those are suspended here by §26 (Static
Conversion Sites)'s own scope test, not omitted by accident. If you find yourself needing one of
them, see *When this stops being a landing* below.

**One part of §20 (Toolchain, Code Quality and Dependency Management) is NOT suspended, and it is
the part that looks least applicable here: the dependency and update policy.** This repository has
no application dependencies and it does have a dependency tree — the actions the gate runs, two of
which hold write permission on the publication origin, and the tools it fetches by pinned version.
`.github/dependabot.yml` proposes their updates and never merges them. **The scaffold this site was built from is dragged rather than watched** — an update mechanism follows a repository's own pipeline, and a scaffold's is content rather than a pipeline, so the rule there is that an action the scaffold and its home repository both use is never pinned lower in the scaffold. Read a major before taking
it: one of these actions changed dotfile handling at a major boundary, and `.nojekyll` is a dotfile
this site stages deliberately, so an upgrade taken at face value removes a published file with a
green build.
...
- **The declaration is `config.controls`**, one key per row of §26 (Static Conversion Sites)'s
  control-placement table, and `check-config.mjs` reads it: a missing row, a key that is not a row,
  a placement outside `edge` / `gate` / `provider` / `document` / `absent`, or an `absent` with no
  reason all fail the gate. The reason is stated as a **capability** — what the host, the receiver
  or the architecture cannot do — never as an intention. Six rows ship answered because they have
  one right answer here; the four left open are the ones the hosting decision settles.
- **A file that does nothing reads as a control.** `_headers` committed to a repository whose host
  serves no custom headers is inert, and from in here it looks exactly like the headers being in
  force. The gate compares both directions: a row claiming the edge with no file, and a file with
  no row claiming it.
...
- **A container is not a measurement.** `config.tagContainerId` delivers events; it does not record
  them. Name the property that receives them in `config.measurementDestination`, and set
  `eventsObserved` only once one has been seen arriving there. A container wired to no destination
  passes every check in this repository and records nothing.
...
- **Indexable is not indexed.** Everything above is verifiable here, and none of it says the site
  was ever found. So either `config.discoverability.indexed` is true — a verified search property
  with the sitemap submitted, before publication, because coverage history starts at verification —
  or it is false, with an owner and a reason, and the generated `robots.txt` says so. An
  unregistered site and a deliberately unlisted one are indistinguishable from inside this
  repository, which is why the decision is written rather than implied. Coverage is read on the
  review cadence; de-indexing a removed page needs that property.
...
- Never weaken a threshold to make a run green. A loosened floor reads exactly like a considered
  ...
  the same commit.
- **A floor this site has not reached is CARRIED, never lowered.** `ratchet-measures.mjs --init`
  records it in `.gate-measures.json`; the floors in `lighthouserc.json` stay exactly as published
  and become the target. Three properties are not optional: only what is below its floor is carried
  (a metric already meeting one is held to the floor, which is stricter); the comparison has a
  tolerance recorded beside the number, because a measurement is not a count and a zero-tolerance
  comparison reddens the gate on runner variance alone; and reaching the floor is one-way — the
  metric leaves the record on the next `--update` and does not come back.
...
governs it, and stretching this section over it is the failure §26 (Static Conversion Sites) was written to prevent.
